...
class Dukpt {
    ...
    _deriveDukptSessionKey(keyMode = 'datakey') {
        ...
        const ipek = this._createIPEK(dBDK, dKSN); // Always start with IPEK
    ...
    _createDataKeyHex(ipek, ksn) {
        const derivedPEK = this._deriveKeyHex(ipek, ksn);
    ...
    _deriveKey(ipek, ksn) {
        ...
        let curKey = ipek;
        const counter = this._getCounter(ksn);
        const pass = 0;
    ...
    _des(key, message, encrypt, mode, iv, padding) {
        ...
        const keys = this._des_createKeys(key);
        let m = 0,
            i,
            j,
            temp,
    ...
    encryptAES(key, data) {
        ...
        const aesCbc = new aesjs.ModeOfOperation.cbc(keyArray, iv);

        const accumulate = function (a, b) {
