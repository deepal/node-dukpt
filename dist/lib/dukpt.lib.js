...
class Dukpt {
    ...
    static _createDataKeyHex(ipek, ksn) {
        ...
        // left half:
        const left = Dukpt._des(Dukpt._EDE3KeyExpand(maskedPEK), maskedPEK.substring(0, 8), true, CBC, iv, null);

        // right half:
        const right = Dukpt._des(Dukpt._EDE3KeyExpand(maskedPEK), maskedPEK.substring(8), true, CBC, iv, null);
    ...
    static _createIPEK(bdk, ksn) {
        ...
        let maskedKSN = DataOperations.ANDdata(DataOperations.hexstringToData('FFFFFFFFFFFFFFE00000'), DataOperations.hexstringToData(ksn)); // this is now binary

        maskedKSN = maskedKSN.substring(0, 8); // take 1st 8 bytes only

        // get LEFT half of IPEK
        let cipher = Dukpt._des(key, maskedKSN, true, /* encrypt */
        CBC, iv, null);
    ...
    static _getCounter(ksn) {
        ...
        const integerValue = (tailbytes.charCodeAt(0) << 16) + (tailbytes.charCodeAt(1) << 8) + tailbytes.charCodeAt(2);
    ...
    static _encryptRegister(key, reg) {
        ...
        const desEncrypted = Dukpt._des(top8, bottom8xorKSN, true, /* encrypt */
        CBC, iv, null);
